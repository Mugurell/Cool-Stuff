...
void print_words_table(Words*, size_t);
...
void print_words_table(Words *head, size_t table_width)
{
    ...
    while (max_rep > 0) {
        ...
        while (current != NULL)
        {
            size_t word_size = strlen(current->word);
            {
                if (current->repetitions > max_rep)
                    // call a small function to make sure this will be centered
                    // +1 because we print the words with delimiters between
                    print_specifier(word_size + 1, '*');
                else
                    print_specifier(word_size + 1, '-');  // for debugging
                // printf("%*c", word_size+1, '-');
            }
            current = current->next;
        }
        putchar('\n');
    ...
    printf("%*s|", (int)timesx_width, "Word");
    current = head;
    while (current != NULL) {
